...
export function useFlashcards({ saveToLibrary } = {}) {
  ...
  const handleGenerate = useCallback(
    async (text, deckName, difficulty = "medio") => {
      ...
      try {
        const { cards, truncated } = await generateFlashcards(
          text.trim(),
          difficulty,
        );
        setState((prev) => ({
          ...
          phase: "study",
          warning: truncated
            ? "Texto longo: apenas os primeiros 24.000 caracteres foram usados."
            : "",
        }));
      } catch (err) {
  ...
  const handleRegenerateDeck = useCallback(
    async (deck, difficulty = "medio") => {
      ...
      try {
        ...
        update({
          phase: "input",
          error: "Não foi possível gerar novos flashcards. Tente novamente.",
        });
      }
    },
    [update],
